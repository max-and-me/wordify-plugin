...
{
    if (!listeners)
        listeners = std::unique_ptr<ContextListenerList> (new ContextListenerList ());
...
void VstGPTContext::onRequestSelectWord (int index)
{
    if (listeners)
    {
        ...
        listeners->forEach (
                            [this, word] (IContextListener* listener) { listener->onRequestLocatorPosChanged (word.begin/10000); });
    }
}
